...
int ProcessKeyInput();
void Dialog();
int IsEventBlock(int id, int v);       //이벤트 칸인지 확인
...
void Dialog()
{
	...
	printf("몇 명이서 플레이하시겠습니까?(z는 확인/x는 취소) ==> ");
	while (result != 1)
	{
		SetCurrentCursorPos(x0 + 55, y0 + 1);
		printf("%d", tmp_num);
	...
	for (int t = 0; t < total_turn; ++t)   //턴
	{
		result = -1;
		SetCurrentCursorPos(x0 + 2, y0 + 1);
		printf("%d번째 턴입니다.진행하려면 z키를 눌러주세요", t + 1);
		while (result != 1)
		{
			result = ProcessKeyInput();
		}
		SetCurrentCursorPos(x0 + 2, y0 + 1);
		printf("                                                                                  ");
		for (int k = 0; k < numPlayer; ++k)  //한 턴에 플레이어 수만큼 진행
		{
			for (int i = 0; i < 6; ++i)    //미리 정해진 DIALOG 순서로 진행
			{
				result = -1;
				if (i == 0)
				{
					SetCurrentCursorPos(x0 + 2, y0 + 1);
					printf("Player%d의 턴입니다. 진행하려면 z키를 눌러주세요", k + 1);
					while (result != 1)
					{
						result = ProcessKeyInput();
					}
				...
				if (i == 3)
				{
					...
					movePlayer(k, p[k].curPos, n);
				}
				if (i == 4)
				{
					event = IsEventBlock(k, p[k].curPos);

					if (event == 1)
					{
						SetCurrentCursorPos(x0 + 2, y0 + 1);
						printf("이벤트 칸입니다. 무작위 이벤트가 발생합니다!");
						result = ProcessKeyInput();
						while (result != 1)
						{
							result = ProcessKeyInput();
				...
				{
					SetCurrentCursorPos(x0 + 2, y0 + 1);
					printf("Player%d의 턴이 종료되었습니다. 진행하려면 z키를 눌러주세요", k + 1);
					result = ProcessKeyInput();
					while (result != 1)
					{
						result = ProcessKeyInput();
...
int IsEventBlock(int id, int v)
{
	//event칸 19개
	if (v == 1 || v == 8 || v == 12 || v == 18 || v == 21 || v == 25 || v == 27 || v == 32 || v == 36 || v == 39
		|| v == 41 || v == 43 || v == 45 || v == 47 || v == 51 || v == 55 || v == 57 || v == 59 || v == 61)
	{
		return 1;
	}
	else
...
{
	/*
	1. 상대(랜덤)로부터 코인 5개 빼앗기(30%)
	2. 상대(랜덤)에게 코인 5개 주기(30%)
	3. 별 얻기 (5%)
	4. 별 잃기 (5%)
	5. 무작위 위치로 텔레포트(20%)
	6. 상대와 위치 교환(10%)
